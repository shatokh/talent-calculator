...
// Описываем группу тестов для страницы класса Стража
test.describe('Guardian Class Page', () => {
  // Тест: Переход на страницу Стража и проверка возврата на стартовую страницу
  test('should navigate to Guardian page and then go back to home', async ({ page }) => {
    ...
    // 2. Находим ссылку для "Стража" через Page Object
    const guardianLink = classSelectionPage.getClassLink('guardian');
    ...
    await Promise.all([
      page.waitForURL(ClassSelectionPage.GUARDIAN_TALENT_URL_REGEX, { timeout: 20000 }), // Ожидаем URL через Page Object
      guardianLink.click() 
    ]);

    // Assert: Проверяем, что текущий URL страницы действительно соответствует ожидаемому
    await expect(page).toHaveURL(ClassSelectionPage.GUARDIAN_TALENT_URL_REGEX);

    // Assert: Ожидаем, пока изображение таланта Стража появится в DOM и станет видимым
    const guardianTalentImage = talentPage.getGuardianTalentImage(); // Используем локатор из Page Object
    await expect(guardianTalentImage).toBeVisible({ timeout: 20000 });
    ...
    // Например, снова проверим ссылку "Страж" через Page Object.
    await expect(guardianLink).toBeVisible();
  });
});